...
export default function ViewFaqs() {
      ...
      Swal.fire({
        ...
        .then((result) => {
          ...
          if (result.isConfirmed) {
            axios.post(`${apiBaseUrl}faq/delete`, { ids: selectedRecord })
              .then((res) => res.data)
              .then((finalres) => {
  ...
  let changeStatus = () => {
    if (selectedRecord.length > 0) {
      axios
        .post(`${apiBaseUrl}faq/change-status`, { ids: selectedRecord })
        .then((res) => res.data)
        .then((finalRes) => {
          if (finalRes._status) {
            getFaq();
      ...
      <div className='w-full min-h-[610px]'>
        ...
        <div className='max-w-[1220px] mx-auto py-5  '>
          <div className='w-full py-3 px-4 bg-slate-100 rounded-t-md border-1 border-slate-400 flex justify-between items-center'>
            <h2 className='text-2xl font-semibold'>View Faq</h2>
          ...
          <table className='w-full'>
            <thead className='w-full bg-[#374151] text-left uppercase'>
              <tr className=' text-sm font-normal text-gray-400'>
                <th className='p-4 w-[130px] flex gap-5'>
                  <input
                    type='checkbox'
                    onChange={allCheck}
                ...
                <th scope='col' className='w-[80px] px-2 py-3 '> S.No.</th>
                <th scope='col' className='px-5 py-3 '> Question</th>
                <th scope='col' className=' px-5 py-3'> 	Answer</th>
...

              {
                data.length < 1
                  ...
                  (
                    data.map((obj, index) => {
                      return (
                        <tr className='bg-gray-800 hover:bg-gray-600 text-left text-gray-400 mb-5'>
                          <th scope='col' className='p-4 w-[130px] '>
                            <input 
                              type='checkbox' 
                              value={obj._id}
                          ...
                          <th scope='col' className='  ps-2'>{index+1}</th>
                          <th scope='col' className='px-5 py-3 text-white'>{obj.question}</th>
                          <th scope='col' className=' py-3 ps-5'>{obj.answer}</th>
                          ...
                          <th className='  ps-2  text-white font-semibold' >
                            ...
                            >
                              <div className='flex justify-center'>
                                <p className='text-white  p-3 mt-3 w-[40px] rounded-[50%] bg-blue-500'>
                                  <FaPen />
                                </p>
                              </div>
