...
export default function AddFaqs() {
  ...
  let handleSubmit=(e)=>{
    ...
    let url = id ? `faq/update/${id}` : 'faq/add'
    ...
    .then((finalRes)=>{
      ...
      if(finalRes._status){
        ...
        // });
        toast.success(id ? 'faq updated successfully!' : 'faq added successfully!')
        setTimeout(()=>{
            navigate('/faqs/view')
        },2000)
  ...
  let updateData=()=>{
    ...
    // })   
      axios.get(`${apiBaseUrl}faq/single-data/${id}`)
      .then((res) => {
        // console.log(res.data._data)
        ...
        <div className='max-w-[1220px] mx-auto py-5  '>
          ...
          <div className='w-full py-3 px-4 rounded-b-md border-1 border-slate-400 border-t-0'>
            <form action="" onSubmit={handleSubmit}>
              <div className='mb-5'>
                <label htmlFor="" className='font-semibold mb-3'>Question</label>
                <input 
                  type="text" 
                  name='question'
                  defaultValue={singledata.question}
                  placeholder='Question' 
                  className='w-full border-2 border-[#ccc] py-2 px-3 rounded-[8px]'/>
              </div>

              <div className='mb-5'>
                <label htmlFor="" className='font-semibold mb-3'>Answer</label>
                <input 
                  type="text" 
                  name='answer'
                  defaultValue={singledata.answer}
                  placeholder='Answer' 
                  className='w-full  border-2 border-[#ccc] pt-2 pb-[150px] px-3 rounded-[8px]'/>
              </div>
