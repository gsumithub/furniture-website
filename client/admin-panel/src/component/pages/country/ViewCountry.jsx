...
export default function ViewCountry() {
      ...
      Swal.fire({
        ...
        .then((result) => {
          ...
          if (result.isConfirmed) {
            axios.post(`${apiBaseUrl}country/delete`, { ids: selectedRecord })
              .then((res) => res.data)
              .then((finalres) => {
  ...
  let changeStatus = () => {
    if (selectedRecord.length > 0) {
      axios
        .post(`${apiBaseUrl}country/change-status`, { ids: selectedRecord })
        .then((res) => res.data)
        .then((finalRes) => {
          if (finalRes._status) {
            getCountry();
    ...
    <div>
      ...
      <div className='w-full min-h-[610px]'>
        <p className='px-6 py-3 border-b-2 border-[#ccc] w-full font-semibold text-gray-700'>
          ...
          <Link to=" " className='hover:text-blue-500'> Location</Link> /
        ...
        <div className='max-w-[1220px] mx-auto py-5  '>
          <div className='w-full py-3 px-4 bg-slate-100 rounded-t-md border-1 border-slate-400 flex justify-between items-center'>
            <h2 className='text-2xl font-semibold'>View Country</h2>
