...
export default function AddSubSubCategory() {
 ...
 // api fatching
  ...
  let handleSubmit = (e) => {
    ...
    let url = id ? `subSubCategory/update/${id}` : 'subSubCategory/add'
    ...
    axios[method](`${apiBaseUrl}${url}`, myData)
      ...
      .then((finalRes) => {
        ...
        if (finalRes._status) {
          ...
          toast.success(id ? 'subSubCategory updated successfully!' : 'subSubCategory added successfully!')
          setTimeout(() => {
  ...
  let updateData=()=>{
    ...
    if(!id) return
      ...
      axios.get(`${apiBaseUrl}subSubCategory/single-data/${id}`)
      .then((res) => {
        // console.log(res)
  ...
  let getSubCategory=(e)=>{
    ...
    axios.get(`${apiBaseUrl}subSubCategory/sub-parent/${parentId}`)
      .then((res) => res.data)
      .then((finalRes) => {
  ...
  useEffect(()=>{
    getSubCategoryEdit(singledata?.parentCategory)
  },[singledata])
          ...
          <div className='w-full py-3 px-4 rounded-b-md border-1 border-slate-400 border-t-0'>
            <form onSubmit={handleSubmit} action="">
              <div className='grid grid-cols-[30%_auto] gap-10 '>
                <div className=''> 
                  <label htmlFor="" className='font-semibold mb-3'>Sub Sub Category Image</label>
                ...
                <div>
                    ...
                    <select 
                      ...
                      <option value="">Select Sub Category</option>
                      {
                        subCategory.map((obj,index)=>{
                          return(
                            <option selected={singledata?.subCategory == obj._id}
                             className='cursor-pointer'  value={obj._id}>{obj.name}</option>
                          )
                        })
                      }
                  ...
                  <div className='mb-5'>
                    <label htmlFor="" className='font-semibold mb-3'>Sub Sub Category Name</label>
                    <input 
                      name='name'
                      type="text" 
