...
export default function ViewWhyChooseUs() {
  ...
  let getWhyChoseUs = () => {
    axios.get(`${apiBaseUrl}whyChoseUs/view`)
      .then((res) => res.data)
      .then((finalRes) => {
      ...
      Swal.fire({
        ...
        .then((result) => {
          ...
          if (result.isConfirmed) {
            axios.post(`${apiBaseUrl}whyChoseUs/delete`, { ids: selectedRecord })
              .then((res) => res.data)
              .then((finalres) => {
            ...
            Swal.fire({
              title: "Deleted!",
              text: "Your Why Chose Us has been deleted.",
              icon: "success"
            });
  ...
  let changeStatus = () => {
    if (selectedRecord.length > 0) {
      axios
        .post(`${apiBaseUrl}whyChoseUs/change-status`, { ids: selectedRecord })
        .then((res) => res.data)
        .then((finalRes) => {
          if (finalRes._status) {
            getWhyChoseUs();
          ...
          <table className='w-full'>
            <thead className='w-full bg-[#374151] text-left uppercase'>
              <tr className=' text-sm font-normal text-gray-400'>
                ...
                <th scope='col' className='ps-6 py-3 w-[200px] '> Title</th>
                    ...
                    data.map((obj, index) => {
                      return (
                        <tr className='bg-gray-800 hover:bg-gray-600 text-left text-gray-400 '>
                          ...
                          <th scope='col' className='ps-6 py-3 w-[200px] text-white'> {obj.title}</th>

                          <th scope='col' className='  ps-2 font-normal'>	{obj.description}</th>
                          <th scope='col' className='flex justify-center  pt-2 ps-2'> <img src={path + obj.image} alt="" width="100"   /></th>
